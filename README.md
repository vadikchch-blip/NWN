...
## First Access (инвайты и бронирования)

- **Создание инвайтов:** положите в корень проекта файл `Список клиентов.xlsx` с колонками «ФИО» (или «Фамилия» и «Имя»), затем выполните: `npm run seed:invites` или `node scripts/seed_first_access_invites.js`. Для каждой строки будет создан уникальный инвайт с периодом доступа (по умолчанию 7 дней; можно задать `FIRST_ACCESS_STARTS_AT` и `FIRST_ACCESS_ENDS_AT` в .env в формате ISO).
- **Админ-страница:** после входа под учёткой администратора откройте [First Access](/admin-first-access.html) — список инвайтов (с кнопкой «Копировать» ссылку) и таблица всех бронирований.
