...
app.post('/api/first-access/admin/restore-expired', requireAdmin, async (req, res) => {
    try {
        ...
        const r1 = await pool.query(
            `WITH candidates AS (
                SELECT r.id, r.invite_id, r.product_id, r.size, COALESCE(r.qty, 1) as qty, r.reserved_at
                FROM first_access_reservations r
            ...
            to_restore AS (
                SELECT DISTINCT ON (invite_id, product_id, size) id, product_id, size, qty
                FROM candidates
                ORDER BY invite_id, product_id, size, reserved_at DESC
            ),
            restored AS (
        ...
        const products = await pool.query(
            `SELECT p.id as product_id, p.article, p.title, p.price_rrc, p.image_key
             ...
             WHERE p.is_active = true
                OR EXISTS (SELECT 1 FROM first_access_reservations r WHERE r.product_id = p.id AND r.status = 'active')
...
function runReservationCron() {
    if (!pool) return;
    // 1) Restore: только 1 на (invite,product,size) — иначе duplicate key
    pool.query(
        `WITH candidates AS (
            SELECT r.id, r.invite_id, r.product_id, r.size, COALESCE(r.qty, 1) as qty, r.reserved_at
            FROM first_access_reservations r
...
// ── Startup: сумка Washed Indigo — забронирована по умолчанию ──
async function ensureBagReserved() {
    if (!pool) return;
    const SYSTEM_TOKEN = 'sys-reserved-nwn-fa';
    const fixedExpires = process.env.RESERVATION_FIXED_EXPIRES_AT || '2026-03-05T17:00:00.000Z';
    try {
        const prod = await pool.query(
            `SELECT id FROM first_access_products WHERE article = 'FW25B16' AND title ILIKE '%Washed Indigo%' LIMIT 1`
        );
        if (prod.rows.length === 0) return;
        const productId = prod.rows[0].id;

        let inv = await pool.query('SELECT id FROM first_access_invites WHERE token = $1', [SYSTEM_TOKEN]);
        if (inv.rows.length === 0) {
            await pool.query(
                `INSERT INTO first_access_invites (token, full_name, access_starts_at, access_ends_at) VALUES ($1, $2, now(), $3)`,
                [SYSTEM_TOKEN, 'Резерв (админ)', fixedExpires]
            );
            inv = await pool.query('SELECT id FROM first_access_invites WHERE token = $1', [SYSTEM_TOKEN]);
        }
        const inviteId = inv.rows[0].id;

        const ex = await pool.query(
            `SELECT 1 FROM first_access_reservations WHERE invite_id = $1 AND product_id = $2 AND size = 'OS' AND status = 'active'`,
            [inviteId, productId]
        );
        ...
        await pool.query(
            `INSERT INTO first_access_reservations (invite_id, product_id, size, qty, status, reserved_at, expires_at)
             VALUES ($1, $2, 'OS', 1, 'active', now(), $3)`,
            [inviteId, productId, fixedExpires]
        );
        await pool.query(
            `UPDATE first_access_product_sizes SET qty_reserved = qty_reserved + 1, updated_at = now() WHERE product_id = $1 AND size = 'OS'`,
            [productId]
        );
        console.log('Bag Washed Indigo marked as reserved');
    } catch (e) { console.error('ensureBagReserved:', e.message); }
}
